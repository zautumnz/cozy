...
func timeTimeout(env *object.Environment, args ...O) O {
	var ms int64
...
func timeInterval(env *object.Environment, args ...O) O {
	var ms int64
	var f *object.Function
	switch t := args[0].(type) {
	case *object.Integer:
	...
	go func() {
		for {
			select {
			case <-ticker.C:
				go ApplyFunction(env, f, make([]O, 0))
			case <-clear:
				ticker.Stop()
				return
			}
